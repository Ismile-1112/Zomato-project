  ...
  return (
    <div className="flex w-full items-center justify-between lg:hidden">
      ...
      <div className="flex items-center gap-3 relative">
        <button className="bg-zomato-500 text-white py-2 px-3 rounded-full">Use App</button>
      ...
      <div className="hidden gap-4 w-full items-center justify-around lg:flex">
        ...
        { reduxState?.user?.fullname ? (
            <div className="relative w-20">
              <div onClick={() => setIsDropDownOpen((prev) => !prev)} className="border p-2 border-gray-300 text-zomato-400 w-full h-20 rounded-full">
                <img 
                  src={gravatar.url(reduxState?.user?.email)}
                  alt={reduxState?.user?.email} 
                  className="w-full h-full rounded-full object-cover"
